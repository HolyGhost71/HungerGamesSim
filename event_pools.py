...
night_weights = [0.6, 0.15, 0.1, 0.5, 0.1]  # Night events skew towards "nightEvent"
...
endgame_night_weights = [0.4, 0.4, 0.2]
...
def get_event_pool(cycle, tributes, startingTributes):
    ...
    elif cycle % 2 == 0 and len(tributes) < len(startingTributes) / 4 or len(tributes) <= 3:
        return endgame_day_event_pool, endgame_day_weights
    
    # Endgame Night phase (low tribute count)
    elif cycle % 2 == 1 and len(tributes) < len(startingTributes) / 4 or len(tributes) <= 3:
